...
PatchNotesDelivered_Notes_1201 = {
	...
	Classes
				...
				• Priest
					> Discipline
						+ Fixed an issue that caused Shadow Mend to not benefit from Binding Heal, Mastery, Protective Light when cast on an ally, Inner Focus, Divine Aegis, Unwavering Will, Surge of Light's mana reduction, Words of the Wise, Improved Flash Heal, and Flash Heal's baseline modifier for Discipline Priest.
				...
				• Rogue
					> Subtlety
						+ Trickster: Fixed an issue that prevented damage from Eviscerate's Apex Shadow Clone from dealing Nimble Flurry damage to nearby targets when created by a Coup de Grace.
				• Warlock
					> Affliction
						+ Soul Harvester: Fixed an issue where Wicked Reaping would be cast multiple times if Nightfall was consumed by Drain Soul and more than 1 target is in combat with the Warlock.
					> Destruction
	...
	Housing
				• Resolved an issue preventing some players from purchasing the Admiral's Bed from Pearl Barlow in Boralus and/or the Dark Iron Expedition Tent from Dedric Sleetshaper in Ironforge.
				• Players who have earned promotional decor can find most of it now properly sold by the promotional decor resupply vendors: Tuuran in Stormwind and Gabbi in Orgrimmar.
				• Developers’ notes: Additional copies of Midnight Epic Edition decor will only be available from Dennia Silvertongue in Silvermoon after the launch of the expansion, when players have access to the Midnight version of Silvermoon City.
